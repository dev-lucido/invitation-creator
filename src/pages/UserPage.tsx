...
// src/pages/UserPage.tsx
// Added: trackEvent('download', ...) call after successful generation

import { useState } from "react";
import {
  Box, Button, Typography, Paper, Grid, CircularProgress, Alert,
  TextField, Stack, Divider, Stepper, Step, StepLabel, Chip, Snackbar,
...
import CheckCircleOutlineIcon from "@mui/icons-material/CheckCircleOutline";
import {
  Template, FieldValues, Language, getVariantUrl, getVariantFields,
} from "../types";
import { useTemplates } from "../hooks/useTemplates";
import TemplatePreview from "../components/TemplatePreview";
import LanguagePicker from "../components/LanguagePicker";
...
const STEPS = ["Choose Template", "Select Language", "Fill In Details", "Download"];
...
export default function UserPage() {
  ...
  const reset = () => {
    setStep(0); setSelected(null); setLanguage(null);
    setValues({}); setGenError(null);
  };

  const handleSelectTemplate = (t: Template) => {
    setSelected(t); setLanguage(null); setValues({}); setStep(1);
  };

  const handleSelectLanguage = (lang: Language) => {
    setLanguage(lang); setValues({}); setStep(2);
  };
  ...
  const previewTemplate =
    selected && previewImageUrl
      ? {
          ...selected,
          imageUrl: previewImageUrl,
          fields: language ? getVariantFields(selected, language) : selected.fields,
        }
      : selected;
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3, md: 4 }, maxWidth: 1100, mx: "auto" }}>
      ...
      <Typography variant="body2" color="text.secondary" mb={3}>
        Pick a template, choose your language, fill in your details, and download.
      </Typography>
      ...
      {isMobile && step < 3 && (
        <Box mb={2}>
          ...
          <MobileStepper variant="dots" steps={STEPS.length} position="static"
            activeStep={step} nextButton={null} backButton={null}
            sx={{ p: 0, mt: 0.5, bgcolor: "transparent" }} />
        </Box>
      )}
        ...
        <>
          ...
          {loading ? (
            ...
            <Paper sx={{ p: { xs: 4, md: 6 }, textAlign: "center", border: "2px dashed", borderColor: "divider" }}>
              <ImageIcon sx={{ fontSize: 48, color: "text.disabled", mb: 1 }} />
              <Typography color="text.secondary">No templates available yet.</Typography>
            </Paper>
          ) : (
            <Grid container spacing={2}>
              {templates.map((t) => (
                <Grid item xs={12} sm={6} md={4} key={t.id}>
                  <Paper variant="outlined" onClick={() => handleSelectTemplate(t)}
                    sx={{
                      overflow: "hidden", cursor: "pointer", transition: "all .2s",
                      "&:hover": { boxShadow: 6, transform: "translateY(-3px)", borderColor: "primary.main" },
                      "&:active": { transform: "translateY(0)" },
                    }}>
                    <Box component="img" src={t.imageUrl} alt={t.name}
                      sx={{ width: "100%", height: { xs: 140, sm: 180 }, objectFit: "cover", display: "block" }} />
                    <Box p={2}>
                      <Typography fontWeight={700} gutterBottom noWrap title={t.name}>{t.name}</Typography>
                      <Box display="flex" gap={0.5} flexWrap="wrap">
                        {[...new Map((t.variants || []).map((v) => [v.lang, v])).values()].map((v) => (
      ...
      {step === 1 && selected && (
        <>
          <Button startIcon={<ArrowBackIcon />} onClick={reset} sx={{ mb: 3 }} size="small">
            Back to Templates
          </Button>
          <Paper variant="outlined" sx={{ p: { xs: 2, md: 3 } }}>
            <Typography variant="h6" fontWeight={700} gutterBottom>{selected.name}</Typography>
            <Divider sx={{ mb: 3 }} />
            <LanguagePicker template={selected} selected={language} onSelect={handleSelectLanguage} />
          </Paper>
        </>
      )}
      ...
      {step === 2 && selected && language && previewTemplate && (
        <>
          <Button startIcon={<ArrowBackIcon />} onClick={() => setStep(1)} sx={{ mb: 2 }} size="small">
            Back to Language
          </Button>
          <Grid container spacing={3} alignItems="flex-start">
            <Grid item xs={12} md={7}>
            ...
            <Grid item xs={12} md={5}>
              <Paper variant="outlined" sx={{ p: { xs: 2, md: 3 } }}>
                <Typography variant="h6" fontWeight={700} gutterBottom noWrap>{selected.name}</Typography>
                <Chip label={language} size="small" sx={{ mb: 2 }} />
                <Divider sx={{ mb: 2 }} />
                {activeFields.length === 0 ? (
                  <Alert severity="info" sx={{ mb: 2 }}>No text fields. Download as-is.</Alert>
                ) : (
                  <Stack spacing={2}>
                    {activeFields.map((field) => (
                      <TextField
      ...
      {step === 3 && (
        <Paper sx={{ p: { xs: 3, md: 6 }, textAlign: "center" }}>
          ...
          <Typography variant="h5" fontWeight={700} gutterBottom>Your Invitation is Ready!</Typography>
          <Typography color="text.secondary" mb={3}>The image has been downloaded to your device.</Typography>
          {previewTemplate && (
            <Box mb={3} mx="auto" sx={{ maxWidth: { xs: "100%", sm: 400 } }}>
              <TemplatePreview template={previewTemplate} values={values} />
            </Box>
          ...
          <Stack direction={{ xs: "column", sm: "row" }} spacing={2} justifyContent="center">
            <Button variant="contained" fullWidth={isMobile} onClick={() => { setStep(2); setGenError(null); }}>
              Edit This Invitation
            </Button>
            <Button variant="outlined" onClick={reset} fullWidth={isMobile}>Start Over</Button>
          </Stack>
        </Paper>
      )}
